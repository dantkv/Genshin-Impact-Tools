...
TODO
更新程序单独打包为一个程序
主程序调用更新程序检测更新后，退出主程序
更新程序下载最新版，解压后替换主程序文件
删除临时文件后调起主程序，完成更新。
"""
...
def getLatestVersionInfo():
    ...
    r = requests.get("https://api.github.com/repos/dantkv/Genshin-Impact-Tools/releases/latest").content.decode("utf-8")
    data = json.loads(r)
...
def upgrade():
    ...
    # TODO 解压并替换文件
    # os.system("Expand-Archive -Path {} -DestinationPath {}".format(download_file_path, ROOT_PATH))
